...
public class Ex06BiDirectionTests {
  ...
  void biDirectionTests() {
    ...
    // 조회
    // @ManyToOne(fetch = FetchType.LAZY): 지연 로딩이므로, 일단 Order만 SELECT하고, OrderItems는 나중에 조회함
    Order findOrder = em.find(Order.class, order.getId()); // 여기선 Order의 SELECT만 날아감
    System.out.println(findOrder.getOrderItems().get(0).getItemName()); // 첫번째 주문 아이템의 이름 조회 (여기서 OrderItem의 SELECT가 날아감)

    // 고아 삭제 테스트
    findOrder.getOrderItems().remove(0); // 첫번째 주문 아이템을 리스트에서 삭제(리스트에는 없는데 실제로는 존재하는 고아: item1 발생) - DELETE 쿼리 생성
