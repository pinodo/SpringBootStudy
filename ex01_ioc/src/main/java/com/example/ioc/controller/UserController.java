...
public class UserController {
  ...
  */
  ...
  // 실무 DI
  // 객체 NPE 방지, 객체 불변성 유지, 순환 참조 방지 (A -> B, B -> A, A -> B, ...) 하기 위해서
  // 필드 선언 시 final 키워드를 추가함

  private final NotificationService notificationService;
  private final ObjectMapper objectMapper;

  // NotificationService 타입의 빈이 2개 이상 있으면 이름(smsNotificationService)으로 구분 가능
  public UserController(@Qualifier("smsNotificationService") NotificationService notificationService, ObjectMapper objectMapper) {
    this.notificationService = notificationService;
...
//
  ...
  public void jsonTest() {
    try {
      ...
      // 2. JSON 문자열 -> 자바 객체 (역질렬화: DeSerialization)
